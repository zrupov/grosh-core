...
// Package grostats implements the network stats reporting service.
package grostats
...
// Service implements an Grosh netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	eth    *eth.Grosh      // Full Grosh service if monitoring a full node
	les    *les.LightGrosh // Light Grosh service if monitoring a light node
...
func New(url string, ethServ *eth.Grosh, lesServ *les.LightGrosh) (*Service, error) {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Grostats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the grostats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to grostats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to grostats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to grostats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to grostats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to grostats")
