...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.grosh", new NodeConfig());
			node.start();
			...
			GroshClient ec = node.getGroshClient();
	...
	// signatures too. Regression test for go-grosh issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "grosh-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/grosh/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.grosh", "github.com/groshproject/grosh-core/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "grosh.aar"), "grosh.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "grosh", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
