...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb grodb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *SecureTrie) Prove(key []byte, fromLevel uint, proofDb grodb.KeyValueWriter) error {
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb grodb.KeyValueReader) (value []byte, nodes int, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
