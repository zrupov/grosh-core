...
type Sync struct {
	database grodb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database grodb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) Commit(dbw grodb.KeyValueWriter) (int, error) {
