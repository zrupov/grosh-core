...
var (
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Grosh and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Grosh address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Grosh light client.
type faucet struct {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Grosh light client protocol
...
// close terminates the Grosh connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Grosh address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/groshproject/grosh-core/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Grosh address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Grosh address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Grosh address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Grosh address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Grosh address,
