...
// Package utils contains internal helper functions for go-grosh commands.
package utils
...
import (
	...
	"github.com/groshproject/grosh-core/dashboard"
	...
	"github.com/groshproject/grosh-core/metrics/influxdb"
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `grosh account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Grosh client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Grosh service: %v", err)
	}
}
...
func RegisterGraphQLService(stack *node.Node, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		return nil, errors.New("no Grosh service")
	}); err != nil {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "grosh.", tagsMap)
		}
	}
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) grodb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb grodb.Database) {
	var err error
...
// new format.
//
// e.g. grosh account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// grosh --keystore /tmp/mykeystore --lightkdf account new
//
