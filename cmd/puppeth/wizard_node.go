...
func (w *wizard) deployNode(boot bool) {
	...
	if infos.grostats == "" {
		...
		infos.grostats = w.readString() + ":" + w.conf.grostats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.grostats)
		infos.grostats = w.readDefaultString(infos.grostats) + ":" + w.conf.grostats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Grosh node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
