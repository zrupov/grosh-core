...
type config struct {
	...
	grostats  string   // Grostats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Grosh services known to be running on servers
...
// converts it to an Grosh address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by grostats, which compares IPs textually, not by value.
