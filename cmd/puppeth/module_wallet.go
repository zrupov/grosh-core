...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'grosh --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec grosh --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --grostats \'{{.Grostats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
