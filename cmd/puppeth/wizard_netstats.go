...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for grostats availability")
	if infos, err := checkGrostats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["grostats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["grostats"] = infos.Report()
		grostats = infos.config
	}
