...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"grostats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "grostats":
				if infos, err := checkGrostats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have grostats running, ask whether to make the secret public or not
	if w.conf.grostats != "" {
		fmt.Println()
		fmt.Println("Include grostats secret on dashboard (y/n)? (default = yes)")
