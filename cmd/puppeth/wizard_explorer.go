...
func (w *wizard) deployExplorer() {
	...
	if w.conf.grostats == "" {
		log.Error("No grostats server configured")
		return
	}
	...
	fmt.Println()
	if infos.node.grostats == "" {
		fmt.Printf("What should the explorer be called on the stats page?\n")
		infos.node.grostats = w.readString() + ":" + w.conf.grostats
	} else {
		fmt.Printf("What should the explorer be called on the stats page? (default = %s)\n", infos.node.grostats)
		infos.node.grostats = w.readDefaultString(infos.node.grostats) + ":" + w.conf.grostats
	}
