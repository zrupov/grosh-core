...
func makecache(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: grosh makecache <block number> <outputdir>`)
	}
...
func makedag(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: grosh makedag <block number> <outputdir>`)
	}
	block, err := strconv.ParseUint(args[0], 0, 64)
	if err != nil {
		utils.Fatalf("Invalid block number: %v", err)
...
You should have received a copy of the GNU General Public License
along with grosh. If not, see <http://www.gnu.org/licenses/>.`)
	return nil
}
