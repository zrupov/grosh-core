...
import (
	...
	"github.com/groshproject/grosh-core/p2p/simulations"
	"github.com/groshproject/grosh-core/p2p/simulations/adapters"
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	grosh, err := eth.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}
	server, err := NewLesServer(grosh, &config)
	if err != nil {
		return nil, err
	}
	grosh.AddLesServer(server)
	return grosh, nil
}
