...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Grosh
...
type wallet struct {
	...
	deriveChain     grosh.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Grosh account
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain grosh.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
