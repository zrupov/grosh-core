...
import (
	...
	"github.com/groshproject/grosh-core/accounts/usbwallet/trezor"
...
// or whether the Grosh app was not started on it.
...
// Grosh address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.GroshAddress)
	if _, err := w.trezorExchange(&trezor.GroshGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.GroshTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.GroshTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
