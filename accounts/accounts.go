...
// Package accounts implements high level Grosh account management.
package accounts
...
// Account represents an Grosh account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Grosh account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain grosh.ChainStateReader)
...
//   keccak256("\x19Grosh Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Grosh Signed Message:\n%d%s", len(data), string(data))
