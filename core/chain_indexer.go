...
type ChainIndexer struct {
	chainDb  grodb.Database      // Chain database to index the data from
	indexDb  grodb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb grodb.Database, indexDb grodb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
