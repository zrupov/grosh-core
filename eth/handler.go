...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Grosh peer", "peer", id)

	// Unregister the peer from the downloader and Grosh peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Grosh protocol")
	...
	log.Info("Grosh protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Grosh peer connected", "name", p.Name())

	// Execute the Grosh handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Grosh handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Grosh peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Grosh message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Grosh sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Grosh network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
