...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Grosh address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Grosh account at a particular block.
    type Account {
    ...
    # Transaction is an Grosh transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Grosh account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Grosh account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Grosh block by number or by hash. If neither is
