...
// PublicGroshAPI provides an API to access Grosh related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicGroshAPI struct {
	b Backend
}

// NewPublicGroshAPI creates a new Grosh protocol API.
func NewPublicGroshAPI(b Backend) *PublicGroshAPI {
	return &PublicGroshAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicGroshAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current Grosh protocol version this node supports
func (s *PublicGroshAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicGroshAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Grosh ECDSA signature for:
// keccack256("\x19Grosh Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/groshproject/grosh-core/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Grosh Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/groshproject/grosh-core/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Grosh signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and grosh.
...
// PublicBlockChainAPI provides an API to access the Grosh blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Grosh blockchain API.
...
//
// https://github.com/grosh/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Grosh APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Grosh APIs exposed over the private
// debugging endpoint.
...
// Version returns the current grosh protocol version.
