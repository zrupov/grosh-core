...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useGroshV = true // Default to use V = 27 or 28, the legacy Grosh format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useGroshV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the Grosh PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useGroshV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Grosh ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Grosh Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useGroshV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
